# scripts/generate_copytrading_installer.py
...
# ==============================================================================
# OpenAlgo + AC Agarwal Multi-Account Copy-Trading Platform Installer
# ==============================================================================
set -e
...
echo -e "${CYAN}"
echo "======================================================================"
echo "    OpenAlgo v2.0 + AC Agarwal Copy-Trading Platform Installer        "
echo "======================================================================"
echo -e "${NC}"
...
if [ -f "$CURRENT_DIR/app.py" ] && [ -d "$CURRENT_DIR/broker" ]; then
  ...
  echo -e "${GREEN}[+] Installing OpenAlgo Copy-Trading to target directory: ${INSTALL_DIR}${NC}"
fi

# ------------------------------------------------------------------------------
# Step 1: Interactive SaaS Client & Broker Configuration Setup
# ------------------------------------------------------------------------------
...
# Read Master Broker API Credentials
if [ -z "$USER_ID" ]; then
  read -p "Enter Master AC Agarwal User ID (e.g. DM933): " USER_ID
fi

if [ -z "$API_KEY" ]; then
  read -p "Enter Master Interactive API Key (BROKER_API_KEY): " API_KEY
fi

if [ -z "$API_SECRET" ]; then
  read -p "Enter Master Interactive API Secret (BROKER_API_SECRET): " API_SECRET
fi

if [ -z "$API_KEY_MARKET" ]; then
  read -p "Enter Master Market Data API Key (BROKER_API_KEY_MARKET): " API_KEY_MARKET
fi

if [ -z "$API_SECRET_MARKET" ]; then
  read -p "Enter Master Market Data API Secret (BROKER_API_SECRET_MARKET): " API_SECRET_MARKET
fi
...
# ------------------------------------------------------------------------------
echo -e "\\n${CYAN}--- Step 3: Preparing OpenAlgo Copy-Trading Repository ---${NC}"
...
# ------------------------------------------------------------------------------
echo -e "\\n${CYAN}--- Step 7: Verifying Module Imports & Initializing Copy Trading Database ---${NC}"
...
'''
    ...
    output_path = os.path.join(root_dir, "deploy_openalgo_copytrading.sh")
    with open(output_path, "w") as f:
        f.write(script_content)

    os.chmod(output_path, 0o755)
    print(f"Generated standalone copy-trading installer: {output_path}")
